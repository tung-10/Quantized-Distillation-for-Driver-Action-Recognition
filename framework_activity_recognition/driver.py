...
def train(config_file):
    ...
    if train_config["modality"] == "train_st" or train_config["modality"] == "train_st_quant" or train_config["modality"] == "train_qd":
        ...
        if not teacher_config.get("skip_replace_last_layer", False):
            teacher_network = replace_last_layer(teacher_network, config_file, teacher_config)

        # load teacher model
        teacher_state_dict = torch.load(teacher_config["checkpoint"], map_location="cpu")
        ...
        elif "model" in teacher_state_dict:
            # VideoMAE repo format
            from collections import OrderedDict
            sd = OrderedDict({
                k.replace("module.", ""): v
                for k, v in teacher_state_dict["model"].items()
            })
            teacher_network.model.load_state_dict(sd, strict=True)
            print("VideoMAE teacher weights loaded.")

        # Freeze teacher
        for p in teacher_network.parameters():
    ...
    resume_cfg = config_file.get('resume', {})
    if resume_cfg.get('use', False):
        ckpt_path = resume_cfg['path']
        logger.info(f"Loading checkpoint from: {ckpt_path}")
        ckpt = torch.load(ckpt_path, map_location='cpu')
        # Load model weights
        if 'student_model_state_dict' in ckpt:
            student_network.load_state_dict(ckpt['student_model_state_dict'])
        elif 'float_student_model_state_dict' in ckpt:
            student_network.load_state_dict(ckpt['float_student_model_state_dict'])
        # Load optimizer
        ...
        resumed_epoch  = ckpt.get('current_epoch', ckpt['epoch'] + 1)
        resumed_recall = ckpt.get('best_recall', 0.0)
        wrapper_input['current_epoch'] = resumed_epoch
        wrapper_input['best_recall']   = resumed_recall
        wrapper_input['max_epochs'] = train_config["epoch"] - resumed_epoch
        logger.info(f"Resumed at epoch {resumed_epoch}, best_recall={resumed_recall:.4f}, epochs left={wrapper_input['max_epochs']}")
...
def test_benchmark(config_file):
    ...
    architecture_state_dict = torch.load(architecture_config["model"] + "/latest_model.pth")
