...
class QuantizationAwareTrainingWrapper():
    ...
    def test(self, test_loader):
        """
        This method represents model testing procedure.
        """
        print("Testing...")

        architecture_to_evaluate = self.student_network.eval()
        if self.quantization_framework is True:
            architecture_to_evaluate = torch.quantization.convert(
                architecture_to_evaluate.to('cpu'), inplace=False
            )

        softmaxFunction = nn.Softmax(dim=1)

        student_class_correct  = torch.zeros(self.num_classes)
        student_class_total_target = torch.zeros(self.num_classes)
        student_class_total_pred   = torch.zeros(self.num_classes)

        teacher_class_correct  = torch.zeros(self.num_classes)
        teacher_class_total_target = torch.zeros(self.num_classes)
        teacher_class_total_pred   = torch.zeros(self.num_classes)

        student_available_set = set()
        teacher_available_set = set()

        use_cuda = torch.cuda.is_available() and not self.quantization_framework
        ...
        with torch.no_grad(): 
            for data in test_loader:
                ...
                # Accumulate student stats bằng tensor ops
                for cls_idx in range(self.num_classes):
                    target_mask = (target == cls_idx)
                    pred_mask   = (student_pred == cls_idx)
                    student_class_correct[cls_idx]      += (student_pred[target_mask] == cls_idx).sum().item()
                    student_class_total_target[cls_idx] += target_mask.sum().item()
                    student_class_total_pred[cls_idx]   += pred_mask.sum().item()

                student_available_set.update(target.cpu().tolist())
                ...
                if self.teacher_network is not None:
                    ...
                    for cls_idx in range(self.num_classes):
                        target_mask = (target == cls_idx)
                        pred_mask   = (teacher_pred == cls_idx)
                        teacher_class_correct[cls_idx]      += (teacher_pred[target_mask] == cls_idx).sum().item()
                        teacher_class_total_target[cls_idx] += target_mask.sum().item()
                        teacher_class_total_pred[cls_idx]   += pred_mask.sum().item()

                    teacher_available_set.update(target.cpu().tolist())

        def compute_metrics(class_correct, class_total_target, class_total_pred, available_set):
            recall_list, precision_list = [], []
            for i in available_set:
                recall_list.append(
                    class_correct[i].item() / class_total_target[i].item()
                    if class_total_target[i] > 0 else 0.0
                )
                precision_list.append(
                    class_correct[i].item() / class_total_pred[i].item()
                    if class_total_pred[i] > 0 else float('nan')
                )
            return np.mean(recall_list), np.nanmean(precision_list), recall_list, precision_list

        s_rec, s_prec, s_rec_list, s_prec_list = compute_metrics(
            student_class_correct, student_class_total_target,
            student_class_total_pred, student_available_set
        )

        if self.teacher_network is not None:
            t_rec, t_prec, t_rec_list, t_prec_list = compute_metrics(
                teacher_class_correct, teacher_class_total_target,
                teacher_class_total_pred, teacher_available_set
            )
            return s_rec, s_prec, s_rec_list, s_prec_list, t_rec, t_prec, t_rec_list, t_prec_list

        return s_rec, s_prec, s_rec_list, s_prec_list, None, None, None, None
    ...
    def save_model(self, epoch, student_network, quantization_framework, optimizer, criterion, annotation, location, teacher_network=None, scheduler=None, quantization_function=None):
        ...
        save_dict['current_epoch'] = self.current_epoch 
        save_dict['best_recall'] = self.best_recall
...
class BenchmarkWrapper():
    ...
    def benchmark(self, test_loader):
        ...
        csv_path = str(self.config_file.get("benchmark_output", "window_predictions.csv"))
        csv_file = open(csv_path, "w", newline="")
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(["clip_path", "win_idx"] + [f"logit_{i}" for i in range(self.num_classes)] + ["predicted", "ground_truth"])
        _win_counter = {}

        for data in test_loader:
            try:
                inputs, target, file_paths = data
            except ValueError:
                inputs, target = data
                file_paths = ["unknown"] * len(target)
        ...
        for data in test_loader:
            ...
            for i in range(len(target.data)):
                ...
                fp = file_paths[i] if isinstance(file_paths, (list, tuple)) else file_paths[i]
                _win_counter[fp] = _win_counter.get(fp, -1) + 1
                logits = network_outputs[i].tolist()
                csv_writer.writerow([fp, _win_counter[fp]] + logits + [network_current_prediction, network_current_target])
        ...
        try:
            inputs_batch, target_batch, _ = next(iter(test_loader))
        except ValueError:
